...
static SHARED_DATA struct notify_data static_notify_data[CONFIG_CORE_COUNT];
...
void init_system_notify(struct sof *sof)
{
	...
	*notify = rzalloc(SOF_MEM_ZONE_SYS, 0, SOF_MEM_CAPS_RAM,
	...
	if (cpu_get_id() == PLATFORM_PRIMARY_CORE_ID)
		sof->notify_data = platform_shared_get(static_notify_data,
						       sizeof(static_notify_data));
}
